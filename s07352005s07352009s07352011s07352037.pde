...
void setup(){
   backgroundMusic = new SoundFile(this, "y26xk-a2bd3.wav"); 
  backgroundMusic.amp(0.25);
  backgroundMusic.play();
  deathNoise = new SoundFile(this, "Death Noise.wav");
  deathNoise.rate(1.10);  
  deathNoise.amp(0.25); 
  hero = new Player(400, 524, 50);
  for(int i = 0; i < 300; i++){
...
void displayControls(){
  ...
  if(timer < 250){
    ...
    textSize(60);
    fill(0);
    text("Tap to jump", 240, 60);
  }else{ //displays current score
    textAlign(CENTER);
    textSize(60);
    fill(0);
    text("Current score: "+timer/60, 220, 60);
  }
}
...
void scenery(){ 
  ...
  img2 = loadImage("b2.jpg");
  image(img,0,0,displayWidth, displayHeight);
  //ground
  strokeWeight(3);
  stroke(0);
  image(img2,0, 550, displayWidth, 50);
}
